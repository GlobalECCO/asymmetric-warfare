...
require([
  ...
  function onSocketUserData(userdata) {
    chatcontroller.init(userdata.username, emit);
  ...
  function onSocketGetVote(vote) {
    ...
    emit('vote', {name: vote.name, choice: choice ? 'yes' : 'no'});
  };

  function onSocketUpdate(data, fromTutorial) {
    ...
    if (((data.state && data.history.length <= 10) || data.history.length === 5) && !Tutorial.isActive() && !Tutorial.isFinished()) {
      Tutorial.setInitialGameState(data);
      Tutorial.beginTutorial();
    }
    else if (Tutorial.isActive() && !Tutorial.isFinished() && fromTutorial === undefined) {
      Tutorial.setInitialGameState(data);
    }
    else {
      if (data.state) {
        ...
        // Auto end the turn if there are no more actions you can perform.
        if (data.state.currentTurn === role && currentTurn.isComplete()) {
          actionscontroller.endTurn();
          return;
        }
        ...
        boardview.render(role, board, data.state.currentPhase, data.state.currentTurn, currentTurn.movementPoints(), data.state.visibleInsurgents, data.state.revealedInsurgents);
        ...
        if (data.state.currentPhase === h.C.PLAYING && currentTurn.id() === h.C.STATE) {
          statusview.displayMovesLeft(currentTurn.movementPoints());
          notifyPlayer();
        }
      } else {
        var history = _.map(data.history, function(dto) {
  ...
  function onSocketInterrogateResult(poskeys) {
    var msg;
  ...
  function updateHistoryView(history, playbackTurn) {
    historyview.render(history, playbackTurn);
    $(".posKey").bind('click', function(event) {
      event.stopPropagation();
    });
    $(".posKey").bind('mouseover', function() {
      var pos = Position($(this).data('circle') + ',' + $(this).data('rank'));
      boardview.highlightPosition(pos);
    });
    $(".posKey").bind('mouseleave', function() {
      boardview.clearHighlightedPosition();
    });
  }

  function presentTurn(history) {
    $('#gameover_banner').remove();

    playbackHistory = history;
    var asymwar = AsymmetricWarfare(history, playbackTurn);
    turnCount = asymwar.getTurnCount();
    if (playbackEnabled) {
      if (playbackTurn === -1) {
        playbackTurn = turnCount;
      }
      $('.playbackContainer #playbackCounter .label').html(playbackTurn + "<hr>" + turnCount);
    }

    var currentTurn = asymwar.currentTurn();
    var currentRole = currentTurn? currentTurn.id() : null;

    // Auto end the turn if there are no more actions you can perform.
    if (!playbackEnabled && currentRole === role && currentTurn.isComplete()) {
      actionscontroller.endTurn();
      return;
    }

    var stateData = asymwar.state(h.C.INSURGENT);

    var movementPoints = currentTurn? currentTurn.movementPoints() : null;
    boardview.render(
        role,
        asymwar.getBoard(),
        ...
        currentRole,
        movementPoints,
        stateData.visibleInsurgents,
        stateData.revealedInsurgents,
        playbackEnabled
    );
    actionsview.render(role, asymwar.canGrow(), asymwar.getBoard(), playbackEnabled? h.C.GAMEOVER: asymwar.currentPhase(), currentTurn);
    updateHistoryView(history, playbackTurn);
    statusview.render(asymwar.currentPhase(), currentRole, role, asymwar.killedInsurgents(), asymwar.reserveInsurgents(), playbackEnabled);

    if (!playbackEnabled && asymwar.currentPhase() === h.C.PLAYING && currentRole === h.C.INSURGENT) {
      statusview.displayMovesLeft(movementPoints);
      notifyPlayer();
    } else {
      statusview.hideMovesLeft();
    }
  ...
  socket.on('interrogate-result', onSocketInterrogateResult);
  ...
  $('.playbackContainer #playbackFirst').bind('click', function(event) {
    playbackTurn = 0;
    presentTurn(playbackHistory);
    event.stopPropagation();
  });
  $('.playbackContainer #playbackLast').bind('click', function(event) {
    playbackTurn = turnCount;
    presentTurn(playbackHistory);
    event.stopPropagation();
  });
  $('.playbackContainer #playbackPrev').bind('click', function(event) {
    playbackTurn--;
    if (playbackTurn < 0) {
      playbackTurn = 0;
    }
  ...
  $('.playbackContainer #playbackNext').bind('click', function(event) {
    playbackTurn++;
    if (playbackTurn > turnCount) {
      playbackTurn = turnCount;
    }
    presentTurn(playbackHistory);
    event.stopPropagation();
  ...
  boardcontroller.init(emit);
  actionscontroller.init(emit);
  boardview.init($('#pieces'), boardcontroller, $("#table_area"), showNotification);
