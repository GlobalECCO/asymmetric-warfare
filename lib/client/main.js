...
require([
    ...
    if (data.history.length === 5 && !Tutorial.isActive() && !Tutorial.isFinished()) {
      ...
      if (data.state) {
        ...
        if (data.state.currentPhase === h.C.PLAYING && currentTurn.id() === h.C.STATE) {
          $('#moves_left').show();
          $('#moves_left #count').text(currentTurn.movementPoints());
          notifyPlayer();
        }
      } else {
        var history = _.map(data.history, function(dto) {
  ...
  function presentTurn(history) {
    ...
    statusview.render(asymwar.currentPhase(), currentRole, role, asymwar.killedInsurgents(), asymwar.reserveInsurgents());

    if (asymwar.currentPhase() === h.C.PLAYING && currentRole === h.C.INSURGENT) {
      $('#moves_left').show();
      $('#moves_left #count').text(movementPoints);
      notifyPlayer();
    }
