...
require([
  ...
  function onSocketUpdate(data, fromTutorial) {
    ...
    if (data.history.length === 5 && !Tutorial.isActive() && !Tutorial.isFinished()) {
      ...
      if (data.state) {
        ...
        _.each(data.state.revealedInsurgents, function(revealed) {
          var position = Position(revealed.pos);
          boardview.revealInsurgents([position], 1.0 - (revealed.time / h.C.INTERROGATE_REVEAL_DURATION));
        })
  ...
  $('.playbackContainer #playbackFirst').bind('click', function() {
    playbackTurn = 0;
    presentTurn(playbackHistory);
  });
  $('.playbackContainer #playbackLast').bind('click', function() {
    playbackTurn = turnCount;
    presentTurn(playbackHistory);
  });
  $('.playbackContainer #playbackPrev').bind('click', function() {
    playbackTurn--;
    if (playbackTurn < 0) {
      playbackTurn = 0;
    }
  ...
  $('.playbackContainer #playbackNext').bind('click', function() {
    playbackTurn++;
