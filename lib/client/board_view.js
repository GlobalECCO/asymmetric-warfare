define(['underscore', 'allong.es', 'lib/constants', 'lib/position', 'lib/insurgent_move', 'lib/state_move'],
  ...
  function modifyHighlight($space, strokeColour, fillColour) {
    ...
    if (data.hasOwnProperty('override')) {
      data.strokeColorOR = strokeColour;
      data.fillColorOR = fillColour;
    } else {
      data.alwaysOn = true;
      data.fill = true;
      data.stroke = true;
  ...
  function flash($space, strokeColor, fromColor, toColor, speed, override) {
    ...
    data[id] = setInterval(function() {
      ...
      if (override) {
        modifyOverrideHighlight($space, strokeColor, rgbMix(fromColor, toColor, alpha));
      } else {
        modifyHighlight($space, strokeColor, rgbMix(fromColor, toColor, alpha));
      }
    }, 100);
  ...
  function markMoveable($space) {
    modifyHighlight($space, "FF7F00", "883800");
    flash($space, "FF7F00", [136, 56, 0], [255, 180, 0], 20);
  }
  ...
  function markSelected($space) {
    if (!isPlayback) {
      modifyHighlight($space, "FF0000", "FF0000");
      $('#selection_map').append($space);
    }
  };

  function markVisible($space) {
    modifyHighlight($space, "DDFF00", "FFFF00");
    flash($space, "DDFF00", [255, 255, 0], [255, 255, 255], 20);
    $('#selection_map').append($space);
  };
  ...
  var BoardView = function() {
    ...
    var revealInsurgents = function(positions, alpha) {
      _.chain(positions)
    ...
    return {
      init: init,
      render: function(_role, _board, _currentPhase, _currentTurn, _remainingMovementPoints, _isPlayback) {
        role = _role;
        board = _board;
