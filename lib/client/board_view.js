define(['underscore', 'allong.es', 'lib/constants', 'lib/position', 'lib/insurgent_move', 'lib/state_move'],
  ...
  function modifyHighlight($space, params) {
    ...
    if (data.hasOwnProperty('override')) {
      ...
      data.stroke = (params.stroke == undefined) ? true : params.stroke;
  ...
  function flash($space, param, speed, override) {
    ...
    data[id] = setInterval(function() {
      ...
      if (override) {
        modifyOverrideHighlight($space, param.fillColor, rgbMix(param.strokeColor[0], param.strokeColor[1], alpha));
      } else {
  ...
  function overrideHighlightSpace($space) {
    ...
    flash($space, {fillColor:"00C8FF", strokeColor:[[0, 200, 255], [255, 255, 255]]}, 20, true);
  };

  function markMoveable($space) {
    flash($space, {fillColor:darkGreen, strokeColor:[[31,76,15], [82,204,41]]}, 20);
  }

  function highlightSpace($space) {
    if (!isPlayback) {
      modifyHighlight($space, {strokeColor:medGreen, fillColor:'777777', fill:true, fillOpacity:0.6});
      flash($space, {fillColor:'777777', strokeColor:[[31,76,15], [82,204,41]]}, 20);
    }
  };

  function markSelected($space) {
    if (!isPlayback) {
      modifyHighlight($space, {strokeColor:medGreen, fillColor:'aaaaaa', fill:true, fillOpacity:0.6});
      $('#selection_map').append($space);
    }
  };

  function markVisible($space) {
    modifyHighlight($space, {fill:true,fillColor:'ffff00', fillOpacity:0.32, strokeColor:medGreen});
    flash($space, {fillColor:'ffff00', strokeColor:[[31,76,15], [82,204,41]]}, 20);
    $('#selection_map').append($space);
  };
  ...
  var BoardView = function() {
    ...
    var render = function(role, board, currentPhase, currentTurn, remainingMovementPoints, visibleInsurgents, revealedInsurgents) {
      ...
      _.each(board.getPieces(), function(pieces_here, poskey) {
        ...
        if (visible) {
          var $space = $findSpace(position);
          modifyHighlight($space, {fill:true,fillColor:'ffff00', fillOpacity:0.32, stroke:false, strokeColor:medGreen});
          //flash($space, {fillColor:'ffff00', strokeColor:[[31,76,15], [82,204,41]]}, 20);
          $('#selection_map').append($space);
        }
