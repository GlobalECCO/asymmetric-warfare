...
function(_, allong, C, Position, InsurgentMove, StateMove) {
  ...
  function markPlaceable($space) {
    ...
    if ((!data.fillStart && !data.fillEnd) || data.fillEnd.opacity <= 0) {
      flashHighlight($space,
        [{fillColor:"FFFFFF", fillOpacity:0.3, fillFade:500},
        {fillColor:"FFFFFF", fillOpacity:0.3, fillFade:500}], 200);
    }
  }
  ...
  function markSelected($space) {
    if (!isPlayback) {
      modifyHighlight($space, {strokeColor:lightGreen, strokeWidth:5, strokeOpacity:1, fillColor:'FFFFFF', fillOpacity:0.5, strokeFade:500, fillFade:500});
