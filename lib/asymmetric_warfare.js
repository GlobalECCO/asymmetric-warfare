define(['underscore', 'lib/helpers'], function(_, h) {
  ...
  var AsymmetricWarfare = function() {
    ...
    var endOfTurn = function() {
      revealedInsurgents = _.map(revealedInsurgents, function(revealed) {
        revealed.time++;
        return revealed;
      })
      .filter(function(revealed) {
        return revealed.time < h.C.INTERROGATE_REVEAL_DURATION;
      });
    ...
    var interrogate = function(interrogate) {
      ...
      _.each(insurgents, function(pos) {
        var posKey = pos.asKey();
        var revealedInsurgent = _.find(revealedInsurgents, function(revealed) {
          return revealed.pos === posKey;
        });

        if (!revealedInsurgent) {
          revealedInsurgents.push({pos: posKey, time: 0});
        } else {
          revealedInsurgent.time = 0;
        }
      });
    ...
    return {
      ...
      stateMove: function(src, dest) {
        ...
        // If we have a revealed insurgent at this location,
        // remove it as the location has no been revealed.
        var destKey = dest.asKey();
        revealedInsurgents = _.filter(revealedInsurgents, function(revealed) {
          return (revealed.pos !== destKey);
        });
      },
      ...
      state: function(role) {
        ...
        return {
          ...
          revealedInsurgents: revealedInsurgents,
  ...
  var AsymmetricWarfareWithHistory = function(_history, _playbackTurn) {
    var history = _history || [];
    var playbackTurn = _playbackTurn > -1? _playbackTurn: -1;
    ...
    if (!history || history.length === 0) {
      ...
      for (var i = 0; i < history.length; i++) {
        ...
        if (entry.type() === h.C.PLACEMENT && entry.player() === h.C.STATE) {
          ...
          if (entry.type() !== h.C.END_TURN) {
            turnCount++;
          }
          if (playbackTurn === -1 || playbackTurn >= turnCount) {
            entry.apply(asymwar);
          }
        }
      }
